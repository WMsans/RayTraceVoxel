...
namespace VoxelEngine.Core.Analysis
{
    public class StructuralIntegritySystem : MonoSingleton<StructuralIntegritySystem>
    {
        ...
        // [FIX] Add a flag to prevent overlapping analysis calls
        ...
        {
            // [FIX] Prevent re-entry. If busy, skip this request.
        ...
        private IEnumerator AnalyzeRoutine(Bounds bounds)
        {
            ...
            if (VoxelVolumePool.Instance == null || VoxelVolumePool.Instance.ActiveChunkCount == 0) 
            {
                _isAnalyzing = false;
                yield break;
            }
            ...
            // --- Buffer Setup ---
            // Only recreate buffer if size changed to avoid GC/Allocation churn
            if (_resultBuffer != null && _resultBuffer.count != totalVoxels)
            {
            ...
            // --- Readback Setup ---
            // Only recreate array if necessary
            if (_readbackArray.IsCreated && _readbackArray.Length != totalVoxels)
            {
                _readbackArray.Dispose();
            }
            if (!_readbackArray.IsCreated)
            {
                _readbackArray = new NativeArray<uint>(totalVoxels, Allocator.Persistent);
            }

            // Request Readback
            // [FIX] Pass parameters via closure carefully
            AsyncGPUReadback.RequestIntoNativeArray(ref _readbackArray, _resultBuffer, (request) => 
            {
                // [FIX] Handle completion and Unlock
                try
                {
                    OnReadbackComplete(request, resX, resY, resZ);
                }
                finally
                {
                    _isAnalyzing = false;
        ...
        private void OnReadbackComplete(AsyncGPUReadbackRequest request, int w, int h, int d)
        {
            if (request.hasError) 
            {
                Debug.LogError("Structural Integrity Readback Error");
                return;
            }
            ...
            {
                // Removed log spam to prevent editor lag
